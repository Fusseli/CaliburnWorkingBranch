...
{
    /// <summary>
    /// Quest for the Flamedancer's Boots artifact.
    /// </summary>
    /// <author>Aredhel</author>
    public class FlamedancersBoots : ArtifactQuest
    {
		...
		{
            ArtifactQuest.Init("Flamedancer's Boots", typeof(FlamedancersBoots));
		}
		...
		public override bool ReceiveItem(GameLiving source, GameLiving target, DbInventoryItem item)
		{
			...
			if (Step == 2 && ArtifactMgr.GetArtifactID(item.Name) == ArtifactID)
			{
				...
				if (versions.Count > 0 && RemoveItem(player, item))
				{
					...
					String reply = String.Format("Here is your boots. Do not lose it, it is irreplaceable.");
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			{
                String reply = String.Format("Flamedancer's Boots is an interesting item. The story of its {0} {1}",
					"making rather unique, don't you think? If you give me the scroll, I'll give you the",
					"boots, with all its ancient powers back.");
				scholar.TurnTo(player);
				scholar.SayTo(player, eChatLoc.CL_PopupWindow, reply);
				Step = 2;
				return true;
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return "Obtain the Aerus Truesight Potion; Kill the waves of centaurs.";
					case 2:
