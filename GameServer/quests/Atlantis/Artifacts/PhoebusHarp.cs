...
{
    /// <summary>
    /// Quest for the Phoebus' Harp artifact.
    /// </summary>
    /// <author>Aredhel</author>
    public class PhoebusHarpNecklace : ArtifactQuest
    {
        public PhoebusHarpNecklace()
			: base() { }

		public PhoebusHarpNecklace(GamePlayer questingPlayer)
			: base(questingPlayer) { }
        ...
        public PhoebusHarpNecklace(GamePlayer questingPlayer, DbQuest dbQuest)
		...
		{
            ArtifactQuest.Init("Phoebus Harp Necklace", typeof(PhoebusHarpNecklace));
		}
		...
		public override bool ReceiveItem(GameLiving source, GameLiving target, DbInventoryItem item)
		{
			...
			if (Step == 2 && ArtifactMgr.GetArtifactID(item.Name) == ArtifactID)
			{
				...
				if (versions.Count > 0 && RemoveItem(player, item))
				{
					...
					String reply = String.Format("Here is your necklace. Do not lose it, it is irreplaceable.");
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			{
                String reply = String.Format("Phoebus Harp is an interesting artifact. The story of its {0} {1}",
					"making rather unique, don't you think? If you give me the scroll, I'll give you the",
					"necklace, with all its ancient powers back.");
				scholar.TurnTo(player);
				scholar.SayTo(player, eChatLoc.CL_PopupWindow, reply);
				Step = 2;
				return true;
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return "Defeat the nereid who has the harp.";
					case 2:
		...
		{
            get { return "Phoebus Harp Necklace"; }
		}
