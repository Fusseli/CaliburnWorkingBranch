...
{
    /// <summary>
    /// Quest for the Snakecharmer's Weapon artifact.
    /// </summary>
    /// <author>Aredhel</author>
    public class TheSnakecharmersWeapon : ArtifactQuest
    {
        public TheSnakecharmersWeapon()
			: base() { }

		public TheSnakecharmersWeapon(GamePlayer questingPlayer)
			: base(questingPlayer) { }
        ...
        public TheSnakecharmersWeapon(GamePlayer questingPlayer, DbQuest dbQuest)
		...
		{
            ArtifactQuest.Init("The Snakecharmer's Weapon", typeof(TheSnakecharmersWeapon));
		}
		...
		public override bool ReceiveItem(GameLiving source, GameLiving target, DbInventoryItem item)
		{
			...
			if (Step == 2 && ArtifactMgr.GetArtifactID(item.Name) == ArtifactID)
			{
				...
				if (versions.Count > 0 && RemoveItem(player, item))
				{
					...
					String reply = String.Format("Here is your weapon. Do not lose it, it is irreplaceable.");
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			{
                String reply = String.Format("The Snakecharmer's Weapon is an interesting piece. The story of its {0} {1}",
					"making rather unique, don't you think? If you give me the scroll, I'll give you the",
					"weapon, with all its ancient powers back.");
				scholar.TurnTo(player);
				scholar.SayTo(player, eChatLoc.CL_PopupWindow, reply);
				Step = 2;
				return true;
		...
		{
            get { return "The Snakecharmer's Weapon"; }
		}
