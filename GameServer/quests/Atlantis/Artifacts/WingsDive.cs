...
{
    /// <summary>
    /// Quest for the Wing's Dive artifact.
    /// </summary>
    /// <author>Aredhel</author>
    public class WingsDive : ArtifactQuest
    {
        public WingsDive()
			: base() { }
		...
		{
            ArtifactQuest.Init("Wing's Dive", typeof(WingsDive));
		}
		...
		public override bool ReceiveItem(GameLiving source, GameLiving target, DbInventoryItem item)
		{
			...
			if (Step == 2 && ArtifactMgr.GetArtifactID(item.Name) == ArtifactID)
			{
				...
				if (versions.Count > 0 && RemoveItem(player, item))
				{
					...
					String reply = String.Format("Here is your leggings. Do not lose it, it is irreplaceable.");
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			{
                String reply = String.Format("Wing's Dive is an interesting item. The story of its {0} {1}",
					"making rather unique, don't you think? If you give me the scroll, I'll give you the",
					"leggings, with all its ancient powers back.");
				scholar.TurnTo(player);
				scholar.SayTo(player, eChatLoc.CL_PopupWindow, reply);
				Step = 2;
				return true;
